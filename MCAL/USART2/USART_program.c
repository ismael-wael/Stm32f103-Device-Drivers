...
void MUSART1_voidTransmit(u8* Word)
{
...
void MUSART1_voidTransmitByte(u8 Copy_u8Byte)
{
	while((GET_BIT((USART1_Ptr -> USART_SR), 5)) == 0)
	/* Insert the character to Data Register */
	USART1_Ptr -> USART_DR = Copy_u8Byte;
	/* Hold for Transmission Completion */
	while((GET_BIT((USART1_Ptr -> USART_SR), 7)) == 0 );
}

void MUSART1_voidTransmitByte1(u8 Copy_u8Byte)
{
	u8 Local_u8Currentletter = 0;
